...
- `make-favicon.js`: draws the brand mark into the two favicon files. Edit the
...
- `favicon.ico`, `favicon-192.png`: the site's icons, from `node make-favicon.js`.
  They are committed because they are what search engines and browsers fetch.
...
The favicon is two files rather than an inline SVG for one reason: the icon in a
search result has to be a real file that Google can crawl, in BMP, GIF, ICO, PNG,
JPEG, PPM or TIFF. SVG is not on that list, and a `data:` URI has no URL to
...
than 48x48. Both are generated by `node make-favicon.js`, and both live at fixed
URLs because Google warns that a favicon URL which moves may be ignored.
